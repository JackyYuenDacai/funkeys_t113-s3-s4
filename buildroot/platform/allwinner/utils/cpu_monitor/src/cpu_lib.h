...
#define DEBUG
#ifdef DEBUG
#define dprintf(fmt...)            if (debug_level >= 1) printf(fmt)
#define B_LOG(fmt...)      		   if (debug_level >= 1) printf(fmt)
#define ERROR(fmt...)              if (debug_level >= 1)  printf(fmt)
#else
#define dprintf(x...)
...
extern s32 fuzzy_search(const char *dir, const char *fuzzy_name, char *target);
...
extern s32 read_proc_thermal_stat(int id);
...
extern s32 read_gpu_vmstat(char *path, int externf, const char *format, const char *scan);
extern s32 read_sys_ion_vmstat(struct ion_vmstat *ion);
