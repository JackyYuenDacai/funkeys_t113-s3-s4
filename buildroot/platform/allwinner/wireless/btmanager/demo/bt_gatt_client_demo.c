...
static void bt_test_gattc_ext_scan_report_cb(btmg_le5_ext_scan_report_t *adv_report)
{
    ...
    if(adv_report->event_type & BTMG_LE5_GAP_SET_EXT_ADV_PROP_LEGACY) {
        printf("[legacy adv] %s, adv type:0x%04X, rssi:%d  %s\n", addrstr, adv_report->event_type, adv_report->rssi, name);
    } else {
        printf("[extend adv] %s, adv type:0x%04X, rssi:%d  %s\n", addrstr, adv_report->event_type, adv_report->rssi, name);
    }
}
...
cmd_tbl_t bt_gattc_cmd_table[] = {
    ...
    { "ext_scan", NULL, cmd_ext_scan, "ext_scan [on, passive, off] [int=0x<hex>] [win=0x<hex>] [wl] [dups/nodups] [all]" },
    { NULL, NULL, NULL, NULL },
};
