...
static int le_set_adv_data(const char *ble_name, uint16_t uuid)
{
    ...
    return bt_manager_le_set_scan_rsp_data(&adv);
}
...
static void ble_config_multi_ext_adv(void)
{
    ...
    // start all adv
    bt_manager_le5_ext_adv_enable(1, 4, ext_adv);
}
...
{
    if(config_ext_adv == 1) {
        BTMG_ERROR("extend advertise don't support it yet");
        return;
    }
    if (argc > 1) {
...
static void cmd_ble_adv(int argc, char *args[])
{
    ...
    if (value || strcmp(args[0], "on") == 0) {
        ...
        if(config_ext_adv == 1) {
            bt_manager_le5_ext_adv_enable(0, 4, ext_adv);
        } else {
            bt_manager_le_enable_adv(false);
        }
    }
