...
static int _bt_init(int profile)
{
    ...
    /*BT*/
    if (bt_pro_info->is_a2dp_sink_enabled || bt_pro_info->is_a2dp_source_enabled ||
        bt_pro_info->is_avrcp_enabled || bt_pro_info->is_hfp_hf_enabled || bt_pro_info->is_sppc_enabled) {
...
int main(int argc, char **argv)
{
    ...
    while ((opt = getopt_long(argc, argv, opts, longopts, NULL)) != -1) {
        ...
        case 'h':
            ...
            return EXIT_SUCCESS;
        case 'v':
            printf("btmanager %s, builed time: %s-%s\n", BTMGVERSION, __DATE__, __TIME__);
            return EXIT_SUCCESS;
        case 'p': {
            size_t i;
            struct {
