...
static int pq_core(pq_request_t* request)
{
    ...
    dlogd("start process pq: cmd=%d, id=%d", packet->cmd, head->id);
    switch (head->id) {
        case HARDWARE_INFO: {
        ...
        case PEAK_SHARP:
            if (packet->cmd == CMD_GET) {
                char* data = packet->data;
                int size = packet->size - sizeof(struct pq_package);
                pqal_->get_peak(data, size);
                request->base.ack_callback((const char*)packet, packet->size);
            } else {
                const char* data = packet->data;
                int size = packet->size - sizeof(struct pq_package);
                pqal_->set_peak(data, size);
                struct pq_package* reply = create_reply_packet(CMD_SET, 0);
                request->base.ack_callback((const char*)reply, reply->size);
            }
            break;
        case LTI_SHARP:
            if (packet->cmd == CMD_GET) {
                char* data = packet->data;
                int size = packet->size - sizeof(struct pq_package);
                pqal_->get_lti(data, size);
                request->base.ack_callback((const char*)packet, packet->size);
            } else {
                const char* data = packet->data;
                int size = packet->size - sizeof(struct pq_package);
                pqal_->set_lti(data, size);
                struct pq_package* reply = create_reply_packet(CMD_SET, 0);
                request->base.ack_callback((const char*)reply, reply->size);
            }
            break;
        ...
        case FCE_BE_CTRL:
            if (packet->cmd == CMD_GET) {
                char* data = packet->data;
                int size = packet->size - sizeof(struct pq_package);
                dlogd("start process FCE_BE_CTRL read");
                pqal_->get_fce_be(data, size);
                request->base.ack_callback((const char*)packet, packet->size);
            } else {
                const char* data = packet->data;
                int size = packet->size - sizeof(struct pq_package);
                dlogd("start process FCE_BE_CTRL write");
                pqal_->set_fce_be(data, size);
                struct pq_package* reply = create_reply_packet(CMD_SET, 0);
                request->base.ack_callback((const char*)reply, reply->size);
            }
            break;
        ...
        case CDC_CTRL:
            if (packet->cmd == CMD_GET) {
                char* data = packet->data;
                int size = packet->size - sizeof(struct pq_package);
                dlogd("start process CDC_CTRL read");
                pqal_->get_cdc(data, size);
                request->base.ack_callback((const char*)packet, packet->size);
            } else {
                const char* data = packet->data;
                int size = packet->size - sizeof(struct pq_package);
                dlogd("start process CDC_CTRL write");
                pqal_->set_cdc(data, size);
                struct pq_package* reply = create_reply_packet(CMD_SET, 0);
                request->base.ack_callback((const char*)reply, reply->size);
                const struct hardware_data* hwdata = pq_hardware_data();
                int index = pqal_->get_packet_store_index(head->id, data, size);
