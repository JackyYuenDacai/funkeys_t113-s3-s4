...
static int LayerConfig(int fd, enum tag_DISP_CMD cmd, struct disp_layer_config2 *pinfo, int screen) {
	...
	args[2] = 1;
	pinfo->info.alpha_mode = 1;
	pinfo->info.alpha_value = 0xFF;
...
static int LayerGetPara(int disp_fd, struct disp_layer_config2 *pinfo, int screen) {
	return LayerConfig(disp_fd, DISP_LAYER_GET_CONFIG2, pinfo, screen);
}

static int LayerSetPara(int disp_fd, struct disp_layer_config2 *pinfo ,int screen) {
	return LayerConfig(disp_fd, DISP_LAYER_SET_CONFIG2, pinfo, screen);
}
#else
static int LayerConfig(int fd, enum tag_DISP_CMD cmd, disp_layer_config *pinfo) {
	DISP_DBG_LOG("(%s %d)\n", __FUNCTION__, __LINE__);
	unsigned long args[4] = { 0 };
	unsigned int ret = RET_OK;
...
static int LayerRender(int fd, unsigned int hlay, renderBuf *picture, int screen) {
	int ret;
#ifdef CONF_KERNEL_IOMMU
...
#endif
	ret = LayerSetPara(fd, &config, screen);

	return ret;
}

static int layer_set_src(int fd, unsigned int hlay, SrcInfo *src,
        renderBuf *picture, int screen) {
	unsigned long args[4] = { 0 };
#ifdef CONF_KERNEL_IOMMU
	struct disp_layer_config2 config;
	memset(&config, 0, sizeof(struct disp_layer_config2));
	...
	config.info.screen_win.x = src->rect.x;
	config.info.screen_win.y = src->rect.y;
	config.info.screen_win.width = src->rect.width;
	config.info.screen_win.height = src->rect.height;
...
#if defined(CONF_KERNEL_VERSION_4_9) || defined(CONF_KERNEL_VERSION_5_4) || defined(CONF_KERNEL_VERSION_5_15)
	/* When setting the buffer address in linux4.9, the layer must be enabled,
	 * otherwise the set buffer address will be cleared */
	config.enable = 1;
#endif

#if defined(CONF_KERNEL_VERSION_5_4) || defined(CONF_KERNEL_VERSION_5_15)
	...
	return LayerSetPara(fd, &config, screen);
}
...
int LayerClose(int fd, unsigned int hlay, int screen) {
#ifdef CONF_KERNEL_IOMMU
	struct disp_layer_config2 config;
	memset(&config, 0, sizeof(struct disp_layer_config2));
#else
...
int LayerRelease(int fd, unsigned int hlay, int screen) {
	int chn = HD2CHN(hlay);
	int lyl = HD2LYL(hlay);
	int ret = LayerClose(fd, hlay, screen);
	if (RET_OK == ret) {
...
int DispWriteData(void *hdl, void *data, int size, videoParam *param) {
	...
	for (int i = 0; i < SCREEN_NUM; i++) {
		if (vout_hd->disp_screen & (1 << i))
			layer_set_src(vout_hd->disp_fd, vout_hd->hlayer, &src_info, &renderbuf, i);
	}
#else
...
#endif
	src_info.rect.x      = vout_hd->rect.x;
	src_info.rect.y      = vout_hd->rect.y;
	src_info.rect.width  = vout_hd->rect.width;
	src_info.rect.height = vout_hd->rect.height;
...
int DispSetEnable(void *hdl, int enable) {
	...
	for (int i = 0; i < SCREEN_NUM; i++) {
		...
		LayerGetPara(vout_hd->disp_fd, &config, i);

		config.enable = enable;
		if ((i == 1) && (vout_hd->disp_screen == 3)) {
			unsigned long ioctlParam[2];
			unsigned int width_screen0, height_screen0, width_screen1, height_screen1;
			ioctlParam[0] = SCREEN_0;
			ioctlParam[1] = 0;
			width_screen0 = ioctl(vout_hd->disp_fd, DISP_GET_SCN_WIDTH, ioctlParam);
			height_screen0 = ioctl(vout_hd->disp_fd, DISP_GET_SCN_HEIGHT, ioctlParam);
			ioctlParam[0] = SCREEN_1;
			ioctlParam[1] = 0;
			width_screen1 = ioctl(vout_hd->disp_fd, DISP_GET_SCN_WIDTH, ioctlParam);
			height_screen1 = ioctl(vout_hd->disp_fd, DISP_GET_SCN_HEIGHT, ioctlParam);

			config.info.screen_win.x = vout_hd->rect.x * width_screen1 / width_screen0;
			config.info.screen_win.y = vout_hd->rect.y * height_screen1 / height_screen0;
			config.info.screen_win.width = vout_hd->rect.width * width_screen1 / width_screen0;
			config.info.screen_win.height = vout_hd->rect.height * height_screen1 / height_screen0;
		}
		else {
		...
		//vout_hd->enable = enable;
			...
			if(!enable)
#endif
				ret = LayerSetPara(vout_hd->disp_fd, &config, i);
		}
	}
#if defined(CONF_KERNEL_VERSION_4_9) || defined(CONF_KERNEL_VERSION_5_4) || defined(CONF_KERNEL_VERSION_5_15)
...
#ifdef CONF_KERNEL_IOMMU
	struct disp_layer_config2 *config = (struct disp_layer_config2*) para;
#else
...
#ifdef SUPPORT_CONFIG_FILE
static int DispConfigSecondScreen(void *hdl) {
	...
	dispOutPort *vout_hd = (dispOutPort*) hdl;
#ifdef __SUNXI_DISPLAY2__
	struct disp_device_config device_config;
	memset(&device_config, 0, sizeof(struct disp_device_config));
	struct uci_ptr ptr;
	char key[32];
	struct uci_context * ctx = uci_alloc_context();
	//uci_load(ctx, DISP_CONFIG_FILE, &pkg)
	//if (uci_lookup_ptr(ctx, &ptr, "libuapi.screen1.DeviceUsed", true) == UCI_OK)
	strncpy(key, "libuapi.screen1.DeviceUsed", sizeof(key));
	if (uci_lookup_ptr(ctx, &ptr, key, true) == UCI_OK && !strcmp("1", ptr.o->v.string)) {
		strncpy(key, "libuapi.screen1.Type", sizeof(key));
		if (uci_lookup_ptr(ctx, &ptr, key, true) == UCI_OK)
			device_config.type = string2int(ptr.o->v.string);
		strncpy(key, "libuapi.screen1.Mode", sizeof(key));
		if (uci_lookup_ptr(ctx, &ptr, key, true) == UCI_OK)
			device_config.mode = string2int(ptr.o->v.string);
		strncpy(key, "libuapi.screen1.Format", sizeof(key));
		if (uci_lookup_ptr(ctx, &ptr, key, true) == UCI_OK)
			device_config.format = string2int(ptr.o->v.string);
		strncpy(key, "libuapi.screen1.Bits", sizeof(key));
		if (uci_lookup_ptr(ctx, &ptr, key, true) == UCI_OK)
			device_config.bits = string2int(ptr.o->v.string);
		strncpy(key, "libuapi.screen1.Eotf", sizeof(key));
		if (uci_lookup_ptr(ctx, &ptr, key, true) == UCI_OK)
			device_config.eotf = string2int(ptr.o->v.string);
		strncpy(key, "libuapi.screen1.Cs", sizeof(key));
		if (uci_lookup_ptr(ctx, &ptr, key, true) == UCI_OK)
			device_config.cs = string2int(ptr.o->v.string);
		strncpy(key, "libuapi.screen1.DviHdmi", sizeof(key));
		if (uci_lookup_ptr(ctx, &ptr, key, true) == UCI_OK)
			device_config.dvi_hdmi = string2int(ptr.o->v.string);
		strncpy(key, "libuapi.screen1.Range", sizeof(key));
		if (uci_lookup_ptr(ctx, &ptr, key, true) == UCI_OK)
			device_config.range = string2int(ptr.o->v.string);
		strncpy(key, "libuapi.screen1.Scan", sizeof(key));
		if (uci_lookup_ptr(ctx, &ptr, key, true) == UCI_OK)
			device_config.scan = string2int(ptr.o->v.string);
		strncpy(key, "libuapi.screen1.AspectRatio", sizeof(key));
		if (uci_lookup_ptr(ctx, &ptr, key, true) == UCI_OK)
			device_config.aspect_ratio = string2int(ptr.o->v.string);
	}

	unsigned long ioctlParam[2];
	ioctlParam[0] = (unsigned long) SCREEN_1;
	ioctlParam[1] = &device_config;
	ret = ioctl(vout_hd->disp_fd, DISP_DEVICE_SET_CONFIG, ioctlParam);
#ifdef DISP_DEBUG
	DISP_DBG_LOG("device_config.type = %d\n", device_config.type);
	DISP_DBG_LOG("device_config.mode = %d\n", device_config.mode);
	DISP_DBG_LOG("device_config.format = %d\n", device_config.format);
	DISP_DBG_LOG("device_config.bits = %d\n", device_config.bits);
	DISP_DBG_LOG("device_config.eotf = %d\n", device_config.eotf);
	DISP_DBG_LOG("device_config.cs = %d\n", device_config.cs);
	DISP_DBG_LOG("device_config.dvi_hdmi = %d\n", device_config.dvi_hdmi);
	DISP_DBG_LOG("device_config.range = %d\n", device_config.range);
	DISP_DBG_LOG("device_config.scan = %d\n", device_config.scan);
	DISP_DBG_LOG("device_config.aspect_ratio = %d\n", device_config.aspect_ratio);
#endif
...
static void DispSelectDisplayScreen(void *hdl)
{
	dispOutPort *vout_hd = (dispOutPort*) hdl;
#ifdef SUPPORT_CONFIG_FILE
	int screen = 0;
	...
	char key[32];
	struct uci_context * ctx = uci_alloc_context();
	strncpy(key, "libuapi.disp_screen.Screen", sizeof(key));
	if (uci_lookup_ptr(ctx, &ptr, key, true) == UCI_OK)
		screen = string2int(ptr.o->v.string);
	if (screen == SCREEN_1_AND_2)
		vout_hd->disp_screen = 3;
	else
		vout_hd->disp_screen = (1 << screen);

	uci_free_context(ctx);
#else
	vout_hd->disp_screen = 1;
#endif
}
...
#ifdef SUPPORT_CONFIG_FILE
	DispConfigSecondScreen(hdl);
#endif
	DispSelectDisplayScreen(hdl);
	DISP_CLAR(ioctlParam);
...
int DispDeinit(void *hdl) {
	...
	for (int i = 0; i < SCREEN_NUM; i++) {
		if (vout_hd->disp_screen & (1 << i))
			LayerRelease(vout_hd->disp_fd, vout_hd->hlayer, i);
	}
